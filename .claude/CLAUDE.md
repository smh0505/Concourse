...
- **Line Endings**: This repo's files use CRLF. When editing or writing files here, use CRLF line endings, not LF.
- **Milestone Tracking**:
...
- **URI-based launches** (e.g. `steam://...`, `com.epicgames.launcher://...`): detected by a `://` substring in `executable_path`, routed through `@tauri-apps/plugin-opener`'s `openUrl()` instead of process spawn (`Command::new` can't execute a URI — this previously caused OS error 123). No child process handle exists this way, so playtime tracking is intentionally skipped rather than faked for these; see `milestones.md` Milestone 7 for the tracked follow-up plan (folder-based process polling, Playnite-style).
