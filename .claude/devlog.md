...
- Big Picture tile title hidden by default, reveals on hover/selection. `BigPictureTile.vue`'s `.tile-title` now sits at `opacity: 0` with a `0.15s ease` transition, flipped to `opacity: 1` by `.tile:hover .tile-title` and `.tile.bp-cover-focused .tile-title` - the latter reusing the existing `bp-cover-focused` class already bound to the `focused` prop, so keyboard/gamepad selection reveals the title exactly like mouse hover does, no separate state needed. A pure CSS change (opacity, not `display`/`visibility`), so the tile's own layout space for the title stays reserved and nothing reflows when it appears. `bun run build`/`cargo check` both clean.
