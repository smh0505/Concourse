...
여러 소스(Steam, Epic, GOG, 수동 등록, 그리고 플러그인을 통한 더 많은 소스)의 게임을 하나의
...
개선 작업(Milestone 14), 컴포넌트 교체 테마를 대체하는 JSON-AST 테마 티어(Milestone
17/19), 공유 스타일 컨벤션 작업(Milestone 18), 앱 + 플러그인/테마 자동 업데이트(Milestone
20), 10개 언어 다국어 지원 및 게임 제목/설명의 오프라인 온디바이스 번역(Milestone 21), 그리고
이 문서 사이트(Milestone 22)까지 모두 완료되었습니다. 위에 나열된 모든 공식 플러그인이
운영 중입니다. 남은 작업으로는 에뮬레이터/ROM 스캐너 플러그인과 추가 소스 플러그인
(Xbox/EA/Ubisoft, Milestone 16)이 있습니다.
...
있지 않습니다. 오프라인 번역 기능(Milestone 21)은 두 종류의 서드파티 콘텐츠를 런타임에
